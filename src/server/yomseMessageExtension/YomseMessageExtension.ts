...
@PreventIframe("/yomseMessageExtension/config.html")
export default class YomseMessageExtension implements IMessagingExtensionMiddlewareProcessor {
    ...
    public async onQuerySettingsUrl(context: TurnContext): Promise<{ title: string, value: string }> {
        return Promise.resolve({
            title: "yomse Message Extension Configuration",
            value: `https://${process.env.PUBLIC_HOSTNAME}/yomseMessageExtension/config.html?name={loginHint}&tenant={tid}&group={groupId}&theme={theme}`
        });
    }
